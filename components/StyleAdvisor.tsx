...
import { streamMessage, isApiKeySet } from '../services/geminiService';
...
const StyleAdvisor: React.FC = () => {
  ...
  const apiKeyExists = isApiKeySet();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const stream = await streamMessage(userMessage.text);
      let text = '';
      ...
      return (
          <div className="fixed bottom-6 right-6">
              <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg relative" role="alert">
                  <strong className="font-bold">Chat Unavailable: </strong>
                  <span className="block sm:inline">API key not configured.</span>
              </div>
          </div>
      );
  }
