...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleClose = () => {
    if (status !== 'loading') {
      onClose();
      // Delay state reset to allow modal to close gracefully
      setTimeout(() => {
        setStatus('idle');
        setError(null);
      ...
      >
        ...
        {status === 'success' && newOrder ? (
            <div className="p-8 text-center">
                ...
                <button onClick={handleClose} className="mt-6 w-full bg-teal-500 text-white font-bold py-3 rounded-md hover:bg-teal-600 transition-colors">
                    Continue Shopping
                </button>
            </div>
        ) : (
